...
kernel void shade_mis(global Ray *rays,				   // 0
                     ...
                     )
{
    ...
    {
        float3 color = ray.throughput * mat.emission;
        if (ray.lastMatType == 2)
        {
            float3 L = ray.direction;

            const float NdotL = ray.NdotL;
            const float LNdotL = dot(normal, -L);
            const float area = t_getArea(vertices[idx.x], vertices[idx.y], vertices[idx.z]);
            const float solidAngle = LNdotL * area / (ray.t * ray.t);
            const float lightPDF = 1.0f / solidAngle;
            const float brdfPDF = NdotL / PI;

            if (lightPDF > 0 && brdfPDF > 0)
            {
    ...
    case (2): // Lambert
    {
        ...
        const float3 baryLight = t_getBaryCoords(lightPos, cNormal, vertices[lvIdx.x], vertices[lvIdx.y], vertices[lvIdx.z]);
        ...
        if (NdotL > 0 && LNdotL > 0)
        {
            ...
            const float lightPDF = 1.0f / solidAngle;
            if (lightPDF > 0 && bPDF > 0)
            {
                const float weight = bPDF + lightPDF;
                const float w1 = bPDF / weight;
                const float w2 = lightPDF / weight;

                sRays[pixelIdx].origin = ray.origin + EPSILON * L;
                sRays[pixelIdx].direction = L;
                sRays[pixelIdx].color = ray.throughput * shadowCol / (w1 * bPDF + w2 * lightPDF);
