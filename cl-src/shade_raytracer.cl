...
kernel void shade_raytracer(global Ray *rays,				 // 0
...
)
{
	...
	case (2): // Lambert
	{
		...
		const float3 baryLight =
			t_getBaryCoords(lightPos, cns[lightIdx], vertices[lvIdx.x], vertices[lvIdx.y], vertices[lvIdx.z]);
		...
		// Add ambient color
		colorBuffer[pixelIdx] += (float4)(ray.throughput * color * 0.01f, 0.0f);
		ray.hit_idx = RAY_MASK_REGENERATE;
		break;
	}
	case (3): // Specular
	...
	if (ray.bounces >= 5 || ray.hit_idx == RAY_MASK_REGENERATE)
	{
		ray.hit_idx = RAY_MASK_REGENERATE;
	...
	ray.origin += EPSILON * ray.direction;
	ray.bounces++;
	if (ray.bounces >= 5 | ray.hit_idx < 0)
	{
		// Stop this ray
		ray.hit_idx = -1;
		ray.throughput = (float3)(0);
