...
Plane::Plane(vec3 normal, float offset, uint matIdx, vec3 dimMin, vec3 dimMax)
{
    ...
    {
        helpNormal = glm::vec3(m_Normal.z, 0.f, -m_Normal.x) /
                     sqrtf(m_Normal.x * m_Normal.x + m_Normal.z * m_Normal.z);
    }
    else
    {
        helpNormal = glm::vec3(0.f, -m_Normal.z, m_Normal.y) /
                     sqrtf(m_Normal.y * m_Normal.y + m_Normal.z * m_Normal.z);
    }
...
void Plane::Intersect(Ray &r) const
{
    ...
    if (p.x < dimMin.x || p.z < dimMin.z || p.x > dimMax.x || p.z > dimMax.z ||
        p.y < dimMin.y || p.y > dimMax.y)
        return;
...
vec3 Plane::GetRandomPointOnSurface(const vec3 &direction, vec3 &lNormal,
 ...
 */
TrianglePlane::TrianglePlane(vec3 topRight, vec3 topLeft, vec3 bottomRight,
                             uint matIndex, SceneObjectList *objectList)
{
    ...
    const auto mat =
        material::MaterialManager::GetInstance()->GetMaterial(matIndex);
...
TrianglePlane::TrianglePlane(vec3 topRight, vec3 topLeft, vec3 bottomRight,
                             uint matIndex, GpuTriangleList *objectList)
{
    // find top-left point
    const float minX = fmin(topRight.x, fmin(topLeft.x, bottomRight.x));
    const float minY = fmin(topRight.y, fmin(topLeft.y, bottomRight.y));
    ...
    if (mat.IsLight())
    {
        objectList->AddLight(GpuTriangle(topRight, topLeft, bottomRight,
                                         m_Normal, m_Normal, m_Normal,
                                         matIndex));
        objectList->AddLight(GpuTriangle(bottomRight, topLeft, bottomLeft,
                                         m_Normal, m_Normal, m_Normal,
                                         matIndex));
    }
    else
    {
        objectList->AddTriangle(GpuTriangle(topRight, topLeft, bottomRight,
                                            m_Normal, m_Normal, m_Normal,
                                            matIndex));
        objectList->AddTriangle(GpuTriangle(bottomRight, topLeft, bottomLeft,
                                            m_Normal, m_Normal, m_Normal,
                                            matIndex));
    }
}

void TrianglePlane::Intersect(Ray &r) const
{
    m_T1->Intersect(r);
    m_T2->Intersect(r);
}
...
glm::vec3 TrianglePlane::GetRandomPointOnSurface(const glm::vec3 &direction,
                                                 glm::vec3 &lNormal,
                                                 RandomGenerator &rng) const
{
    if (rng.Rand(1.f) <= .5f)
...
glm::vec3 TrianglePlane::GetNormal(const glm::vec3 &hitPoint) const
{
    return m_Normal;
}
