...
{
  public:
	virtual void TraceRay(core::Ray &r) const = 0;

	virtual bool TraceShadowRay(core::Ray &r, float tMax) const = 0;
	...
	virtual unsigned int TraceDebug(core::Ray &r) const = 0;
};
...
class SceneObjectList : public WorldScene
{
  public:
	...
	inline unsigned int GetPrimitiveCount() override { return static_cast<unsigned int>(m_List.size()); }

	inline bvh::AABB GetNodeBounds(unsigned int index) override { return m_List[index]->GetBounds(); }

	inline unsigned int TraceDebug(core::Ray &r) const override
	{
