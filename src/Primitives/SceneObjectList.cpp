...
const std::vector<SceneObject *> &SceneObjectList::GetLights() const
{
    return m_Lights;
}

void SceneObjectList::TraceRay(Ray &r) const
{
...
const std::vector<SceneObject *> &SceneObjectList::GetObjects() const
{
    return m_List;
}

const std::vector<bvh::AABB> &SceneObjectList::GetAABBs() const
{
    return m_Aabbs;
}

bool SceneObjectList::TraceShadowRay(Ray &r, float tMax) const
{
    for (SceneObject *object : m_List)
    {
        object->Intersect(r);
