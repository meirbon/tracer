...
Torus::Torus(glm::vec3 center, glm::vec3 vAxis, glm::vec3 hAxis, float innerRadius, float outerRadius,
			 unsigned int matIdx)
{
...
void Torus::Intersect(core::Ray &r) const
{
	...
	const double centerToRayOriginDotDirection = dot(r.direction, centerToRayOrigin);
	const double centerToRayOriginDotDirectionSquared = dot(centerToRayOrigin, centerToRayOrigin);
	...
	const double b = 2 * (dot(centerToRayOrigin, r.direction) - axisDotCenterToRayOrigin * axisDotRayDirection);
	const double c = centerToRayOriginDotDirectionSquared - axisDotCenterToRayOrigin * axisDotCenterToRayOrigin;
	const double d = centerToRayOriginDotDirectionSquared + outerRadius2 - innerRadius2;
...
{
	return {centroid + vec3(-1.f, -1.f, -1.f) * (m_OuterRadius + 2 * m_InnerRadius) - EPSILON,
			centroid + vec3(1.f, 1.f, 1.f) * (m_OuterRadius + 2 * m_InnerRadius) + EPSILON};
}

vec3 Torus::GetRandomPointOnSurface(const vec3 &direction, vec3 &lNormal, RandomGenerator &rng) const
{
	return glm::vec3(0.f);
}
...
glm::vec3 Torus::GetNormal(const glm::vec3 &hitPoint) const
{
	...
	const vec3 centerToProjectedHit = normalize(projectedHitpoint - centroid) * m_OuterRadius;
...
glm::vec2 Torus::GetTexCoords(const glm::vec3 &hitPoint) const
{
	...
	const auto offsetDiff = float(m_TorusPlaneOffset - offsetHitPlane);
	const vec3 projectedHitpoint = hitPoint + (m_Axis * offsetDiff);
	const vec3 centerToProjectedHitNormalized = normalize(projectedHitpoint - centroid);
	const vec3 centerToProjectedHit = centerToProjectedHitNormalized * m_OuterRadius;
	const vec3 projectedHit = centroid + centerToProjectedHit;
