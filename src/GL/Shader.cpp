...
// textFileRead
// Store the contents of a text file in a string variable.
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
void Shader::SetInputTexture(unsigned int slot, const char *name,
                             Texture *texture)
{
...
void Shader::setUniform2f(const char *name, vec2 value)
{
...
void ComputeShader::Run(unsigned int group_x, unsigned int group_y,
                        unsigned int group_z)
{
    glDispatchCompute(group_x, group_y, group_z);
}

GLint ComputeShader::getUniformLocation(const char *name)
{
    return glGetUniformLocation(this->m_Id, name);
}

void ComputeShader::setInputTexture(unsigned int slot, const char *name,
                                    Texture *texture)
{
    glActiveTexture(slot);
    texture->Bind();
...
void ComputeShader::setInputBuffer(unsigned int slot, const char *name,
...
void ComputeShader::setUniform4f(const char *name, float x, float y, float z,
                                 float w)
{
    glUniform4f(getUniformLocation(name), x, y, z, w);
}
...
{
    glUniformMatrix4fv(getUniformLocation(name), 1, GL_FALSE,
                       (const GLfloat *)glm::value_ptr(value));
}
