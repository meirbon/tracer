    ...
    : m_Width(width), m_Height(height)
{
    ...
    {
        const auto byteCount = width * height * sizeof(Pixel);
        ...
        glBufferData(GL_PIXEL_UNPACK_BUFFER_ARB, byteCount, data,
                     GL_STREAM_DRAW_ARB);
    ...
    else if (type == FLOAT_SURFACE)
    {
        ...
        glGenBuffers(1, &m_PboId);
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER_ARB, m_PboId);
        glBufferData(GL_PIXEL_UNPACK_BUFFER_ARB, width * height * 4, data,
                     GL_STREAM_DRAW_ARB);
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER_ARB, 0);
    }
    else if (type == FLOAT)
...
Texture::Texture(const char *fileName)
{
    ...
    if (!tmp)
    {
        utils::WarningMessage(__FILE__, __LINE__, fileName, "File not found");
    }
    ...
    for (unsigned int y = 0; y < height; y++)
    {
        ...
        {
            data[y * width + x] = (line[x * 3 + 2] << 16) +
                                  (line[x * 3 + 1] << 8) + line[x * 3 + 0];
        }
    }
    FreeImage_Unload(dib);
    glTexImage2D(textureType, 0, GL_RGBA8, width, height, 0, GL_BGRA,
...
Pixel *Texture::mapToPixelBuffer()
{
...
FPixel *Texture::mapToFPixelBuffer()
{
    glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER_ARB);
    Bind();
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER_ARB, m_PboId);
