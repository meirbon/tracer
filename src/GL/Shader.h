...
{
  public:
	ComputeShader(const char *cfile);
	...
	void Run(unsigned int group_x = 8, unsigned int group_y = 8, unsigned int group_z = 1);

	GLint getUniformLocation(const char *name);

	void setInputTexture(unsigned int slot, const char *name, Texture *texture);

	void setInputBuffer(unsigned int slot, const char *name, ComputeBuffer *buffer);
  ...
  private:
	GLuint m_Id;	  // shader program identifier
	GLuint m_Compute; // compute shader identifier
};
} // namespace gl