...
void CheckGL()
{
	...
	if (error != GL_NO_ERROR)
	{
		...
		utils::WarningMessage(__FILE__, __LINE__, "OpenGL error");
	}
}
...
void CheckShader(GLuint shader)
{
	...
	if (status != GL_TRUE) // thanks Rik
	{
		...
		utils::WarningMessage(__FILE__, __LINE__, buffer, "Shader compile error");
	}
}
...
void CheckProgram(GLuint id)
{
	...
	if (length > 0)
	{
		...
		{
			utils::WarningMessage(__FILE__, __LINE__, buffer, "Shader link error");
		}
		else if (!strstr(buffer, "No errors"))
		{
			utils::FatalError(__FILE__, __LINE__, buffer, "Shader link error");
		}
	}
}
