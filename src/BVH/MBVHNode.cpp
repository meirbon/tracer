...
unsigned int MBVHNode::IntersectDebug(Ray &r, const __m128 &dirx4,
                                      const __m128 &diry4, const __m128 &dirz4,
                                      const __m128 &orgx4, const __m128 &orgy4,
                                      const __m128 &orgz4,
                                      const MBVHNode *pool) const
{
    ...
    const int result = _mm_movemask_ps(
        _mm_and_ps(_mm_cmpge_ps(tmax4, _mm_setzero_ps()),
                   _mm_and_ps(_mm_cmplt_ps(tmin4, tmax4),
                              _mm_cmplt_ps(tmin4, _mm_set1_ps(r.t)))));
    ...
    if (result > 0)
    {
        ...
        for (unsigned int t : tmini)
        {
            ...
            {
                depth += pool[this->child[idx]].IntersectDebug(
                    r, dirx4, diry4, dirz4, orgx4, orgy4, orgz4, pool);
            }
        }
    }
...
void MBVHNode::Intersect(Ray &r, const __m128 &dirx4, const __m128 &diry4,
                         const __m128 &dirz4, const __m128 &orgx4,
                         const __m128 &orgy4, const __m128 &orgz4,
                         const MBVHNode *pool,
    ...
    if (result > 0)
    {
        ...
        for (unsigned int t : tmini)
        {
            ...
            { // leaf node
                ...
                {
                    const uint &primIdx =
                        primitiveIndices[i + this->child[idx]];
                    objectList[primIdx]->Intersect(r);
                }
            }
            else
            {
                pool[this->child[idx]].Intersect(r, dirx4, diry4, dirz4, orgx4,
                                                 orgy4, orgz4, pool,
                                                 primitiveIndices, objectList);
            }
        }
    }
}

void MBVHNode::MergeNodes(const BVHNode &node, const BVHNode *bvhPool,
                          MBVHTree *bvhTree)
{
    int numChildren;
    ...
    for (int idx = 0; idx < numChildren; idx++)
    {
        ...
        { // not a leaf
            ...
            else
            {
                ...
                this->child[idx] =
                    newIdx; // replace BVHNode idx with MBVHNode idx
                this->count[idx] = -1;
                this->SetBounds(idx, curNode.bounds);
...
void MBVHNode::MergeNodesMT(const BVHNode &node, const BVHNode *bvhPool,
                            MBVHTree *bvhTree, bool thread)
{
    int numChildren;
    GetBVHNodeInfo(node, bvhPool, numChildren);
        ...
        { // not a leaf
            ...
            else
            {
                ...
                threads.push_back(bvhTree->m_OriginalTree->m_ThreadPool->push(
                    [newNode, curNode, bvhPool, bvhTree](int) {
                        newNode->MergeNodesMT(*curNode, bvhPool, bvhTree);
                    }));
            }
        }
    }
...
void MBVHNode::MergeNodes(const bvh::BVHNode &node,
...
void MBVHNode::MergeNodesMT(const bvh::BVHNode &node,
                            const std::vector<bvh::BVHNode> &bvhPool,
                            MBVHTree *bvhTree, bool thread)
{
    int numChildren;
...
void MBVHNode::GetBVHNodeInfo(const BVHNode &node, const BVHNode *pool,
                              int &numChildren)
{
    // Starting values
    ...
    if (node.IsLeaf())
    {
        utils::WarningMessage(__FILE__, __LINE__,
                              "This node shouldn't be a leaf.", "MBVHNode");
        return;
    }
...
void MBVHNode::GetBVHNodeInfo(const BVHNode &node,
                              const std::vector<bvh::BVHNode> &pool,
                              int &numChildren)
{
    // starting values
    child[0] = child[1] = child[2] = child[3] = -1;
...
void MBVHNode::SortResults(const float *tmin, int &a, int &b, int &c,
                           int &d) const
{
    if (tmin[a] > tmin[b])
    {
...
MBVHHit MBVHNode::Intersect(Ray &r, const __m128 &dirX, const __m128 &dirY,
                            ...
                            const __m128 &orgY, const __m128 &orgZ) const
{
    ...
    mHit.result = _mm_movemask_ps(
        _mm_and_ps(_mm_cmpge_ps(tmax4, zerops),
                   _mm_and_ps(_mm_cmple_ps(mHit.tmin4, tmax4),
                              _mm_cmplt_ps(mHit.tmin4, _mm_set1_ps(r.t)))));
