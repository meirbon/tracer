...
bool GameObjectNode::Intersect(const core::Ray &r, float &tnear, float &tfar) const
{
	...
	const __m128 t1 = _mm_mul_ps(_mm_sub_ps(boundsWorldSpace.bmin4, org4), dirInversed);
	const __m128 t2 = _mm_mul_ps(_mm_sub_ps(boundsWorldSpace.bmax4, org4), dirInversed);
...
void GameObjectNode::Traverse(core::Ray &rOrg) const
{
	...
	if (r.IsValid())
	{
		rOrg.t = r.t;
		rOrg.obj = r.obj;
		const vec4 normal = inverseMat * vec4(r.obj->GetNormal(r.GetHitpoint()), 0.f);
...
void GameObjectNode::TraverseShadow(core::Ray &rOrg) const
{
	const vec4 org = transformationMat * vec4(rOrg.origin, 1.f);
	const vec4 dir = transformationMat * vec4(rOrg.direction, 0.f);
	core::Ray r = {{org.x, org.y, org.z}, {dir.x, dir.y, dir.z}};
...
GameObjectNode::GameObjectNode(glm::mat4 transformationMat, glm::mat4 inverseMat, bvh::GameObject *gameObject)
{
...
void GameObjectNode::ConstructWorldBounds()
{
	...
	const float minX = glm::min(
		p1.x, glm::min(p2.x, glm::min(p3.x, glm::min(p4.x, glm::min(p5.x, glm::min(p6.x, glm::min(p7.x, p8.x)))))));
	const float minY = glm::min(
		p1.y, glm::min(p2.y, glm::min(p3.y, glm::min(p4.y, glm::min(p5.y, glm::min(p6.y, glm::min(p7.y, p8.y)))))));
	const float minZ = glm::min(
		p1.z, glm::min(p2.z, glm::min(p3.z, glm::min(p4.z, glm::min(p5.z, glm::min(p6.z, glm::min(p7.z, p8.z)))))));

	const float maxX = glm::max(
		p1.x, glm::max(p2.x, glm::max(p3.x, glm::max(p4.x, glm::max(p5.x, glm::max(p6.x, glm::max(p7.x, p8.x)))))));
	const float maxY = glm::max(
		p1.y, glm::max(p2.y, glm::max(p3.y, glm::max(p4.y, glm::max(p5.y, glm::max(p6.y, glm::max(p7.y, p8.y)))))));
	const float maxZ = glm::max(
		p1.z, glm::max(p2.z, glm::max(p3.z, glm::max(p4.z, glm::max(p5.z, glm::max(p6.z, glm::max(p7.z, p8.z)))))));
