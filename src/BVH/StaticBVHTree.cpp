...
namespace bvh
{
bvh::StaticBVHTree::StaticBVHTree(SceneObjectList *objectList, BVHType type,
...
bvh::StaticBVHTree::StaticBVHTree(GpuTriangleList *objectList, BVHType type,
                                  ctpl::ThreadPool *pool)
{
...
#if PRINT_BUILD_TIME
        std::cout << "Building BVH took: " << t.elapsed() << " ms."
                  << std::endl;
#endif
        CanUseBVH = true;
    }
...
bvh::BVHNode &bvh::StaticBVHTree::GetNode(unsigned int idx)
{
    return m_BVHPool[idx];
}

void bvh::StaticBVHTree::TraceRay(Ray &r) const
{
    if (CanUseBVH)
    {
...
bool bvh::StaticBVHTree::TraceShadowRay(Ray &r, float tMax) const
{
    if (CanUseBVH)
    {
        if (m_BVHPool[0].IntersectSIMD(r))
...
void bvh::StaticBVHTree::IntersectWithStack(Ray &r) const
{
    ...
    while (stackptr >= 0)
    {
        ...
        { // Not a leaf
            const bool hitLeftNode =
                m_BVHPool[leftFirst].IntersectSIMD(r, t1near, t1far);
            const bool hitRightNode =
                m_BVHPool[right].IntersectSIMD(r, t2near, t2far);
...
const std::vector<SceneObject *> &bvh::StaticBVHTree::GetLights() const
{
    return m_ObjectList->GetLights();
}

unsigned int bvh::StaticBVHTree::GetPrimitiveCount()
{
    return static_cast<unsigned int>(m_PrimitiveCount);
}

unsigned int StaticBVHTree::TraceDebug(Ray &r) const
{
    if (CanUseBVH)
    {
        if (m_BVHPool[0].IntersectSIMD(r))
...
AABB bvh::StaticBVHTree::GetNodeBounds(unsigned int index)
{
