...
class AABB
{
  public:
    ...
    bool Contains(const __m128 &p) const
    {
        ...
        return ((va[0] >= 0) && (va[1] >= 0) && (va[2] >= 0) && (vb[0] >= 0) &&
                (vb[1] >= 0) && (vb[2] >= 0));
    }
    ...
    inline float Extend(const int axis) const
    {
    ...
    {
        float tmp_x = bmax[0] - bmin[0];
        float tmp_y = bmax[1] - bmin[1];
        float tmp_z = bmax[2] - bmin[2];

        return glm::max(0.0f, tmp_x * tmp_y + tmp_x * tmp_z + tmp_y * tmp_z);
        ...
        //			return fmax(0.0f, e[0] * e[1] + e[0] * e[2] + e[1]
        //* e[2]);
    }
    ...
    inline glm::vec3 Lengths() const
    {
        ...
        //			};
        //			length4 = _mm_sub_ps(this->bmax4, this->bmin4);
        //			return glm::vec3(length[0], length[1],
        // length[2]);
    }
    ...
    union {
        ...
        __m128 bounds[2] = {_mm_set_ps(1e34f, 1e34f, 1e34f, 0),
                            _mm_set_ps(-1e34f, -1e34f, -1e34f, 0)};
    };
    ...
    inline float Center(unsigned int axis) const
    {
