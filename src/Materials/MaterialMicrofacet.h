...
class MaterialMicrofacet
{
  public:
    ...
    vec3 SampleNormal(const vec3 &inDirection, const vec3 &realNormal,
                      RandomGenerator &rng) const;
    float Weight(const glm::vec3 &inDirection, const glm::vec3 &realNormal,
                 const glm::vec3 outDirection) const;

    inline float G(const vec3 &out, const vec3 &in) const
    {
        return G1(out) * G1(in);
    }
