...
{
    return sqrtf(glm::max(FLT_MIN, 1.0f - cos2Theta(w)));
}
...
inline float mf_erf(float x)
{
    ...
    const float y = 1.0f - (((((a5 * t + a4) * t) + a3) * t + a2) * t + a1) *
                               t * expf(-x * x);
  ...
  public:
    ...
    inline glm::vec3 sampleWm(RandomGenerator &rng) const
    {
        ...
        const float sinTheta =
            sqrtf(glm::max(0.0f, 1.0f - cosTheta * cosTheta));
        glm::vec3 wm =
    ...
    inline float lambda(const glm::vec3 &wo) const
    {
        ...
        const float alpha = sqrtf(cos2Phi(wo) * m_AlphaX * m_AlphaX +
                                  sin2Phi(wo) * m_AlphaY * m_AlphaY);
    ...
    inline float G1(const glm::vec3 &wo) const
    {
    ...
    // Weighting function
    inline float weight(const glm::vec3 &wo, const glm::vec3 &wi,
                        const glm::vec3 &wm) const
    {
        return fabsf(glm::dot(wi, wm)) * G(wo, wi) /
               std::max(1.0e-8f, fabsf(cosTheta(wi) * cosTheta(wm)));
    }
