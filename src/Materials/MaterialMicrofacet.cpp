...
MaterialMicrofacet::MaterialMicrofacet(float alphaX, float alphaY)
...
glm::vec3 MaterialMicrofacet::SampleNormal(const vec3 &inDirection,
                                           const vec3 &normal,
                                           RandomGenerator &rng) const
{
    ...
    return localToWorld(vec3(x, y, z), Nt, Nb, normal);
}
...
inline float geometric_term(float ndotv, float ndotl, float ndoth, float hdotv)
{
    ...
    return glm::min(
        1.0f, glm::min(frac2ndoth_hdtov * ndotv, frac2ndoth_hdtov * ndotl));
}
...
float MaterialMicrofacet::Weight(const glm::vec3 &inDirection,
                                 const glm::vec3 &realNormal,
                                 const glm::vec3 outDirection) const
{
    ...
    const float normal_distribution =
        distribution_for_normal(ndoth, (m_AlphaX + m_AlphaY) / 2.0f);
...
float MaterialMicrofacet::lambda(const glm::vec3 outDirection) const
{
    ...
    const float alpha = sqrtf(cos2_phi(outDirection) * m_AlphaX * m_AlphaX +
                              sin2_phi(outDirection) * m_AlphaY * m_AlphaY);
    const float alpha2Tan2Theta =
        (alpha * absTanThetaO) * (alpha * absTanThetaO);
