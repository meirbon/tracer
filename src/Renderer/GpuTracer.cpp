...
GpuTracer::GpuTracer(GpuTriangleList *objectList, Texture *targetTexture1,
                     Texture *targetTexture2, Camera *camera, Surface *skyBox,
                     ctpl::ThreadPool *pool)
    : m_Camera(camera)
{
    Kernel::InitCL();
    m_BVHTree =
        new bvh::StaticBVHTree(objectList, bvh::BVHType::SAH_BINNING, pool);
    ...
    generateRayKernel =
        new Kernel("programs/program.cl", "generateRays", workSize, localSize);
    intersectRaysKernelRef =
        new Kernel("programs/program.cl", "intersectRays", workSize, localSize);
    intersectRaysKernelMF = new Kernel("programs/program.cl", "intersectRaysMF",
                                       workSize, localSize);
    intersectRaysKernelBVH = new Kernel(
        "programs/program.cl", "intersectRaysBVH", workSize, localSize);
    intersectRaysKernelOpt = new Kernel(
        "programs/program.cl", "intersectRaysOpt", workSize, localSize);
    drawKernel = new Kernel("programs/program.cl", "Draw", workSize, localSize);

    wIntersectKernel =
        new Kernel("programs/program.cl", "intersect", workSize, localSize);
    wShadeKernel =
        new Kernel("programs/program.cl", "shade", workSize, localSize);
    wDrawKernel =
        new Kernel("programs/program.cl", "draw", workSize, localSize);
...
void GpuTracer::Render(Surface *)
{
    ...
    const glm::vec3 u =
        normalize(cross(m_Camera->GetViewDirection(), vec3(0, 1, 0)));
    const glm::vec3 v = normalize(cross(u, m_Camera->GetViewDirection()));
    const glm::vec3 horizontal =
        u * m_Camera->GetFOVDistance() * m_Camera->GetAspectRatio();
...
void GpuTracer::SetupSeeds(int width, int height)
{
    ...
    seedBuffer =
        new Buffer(width * height * sizeof(unsigned int), seeds.data());
...
void GpuTracer::SetupObjects()
{
    ...
    primitiveIndicesBuffer =
        new Buffer(static_cast<unsigned int>(m_BVHTree->m_PrimitiveCount) *
                       sizeof(unsigned int),
...
#if MBVH
    BVHNodeBuffer =
        new Buffer(sizeof(bvh::BVHNode), m_BVHTree->m_BVHPool.data());
#else
    BVHNodeBuffer = new Buffer(m_BVHTree->m_BVHPool.size() * sizeof(BVHNode),
                               m_BVHTree->m_BVHPool.data());
#endif
    BVHNodeBuffer->CopyToDevice();

    MBVHNodeBuffer =
        new Buffer((m_MBVHTree->m_FinalPtr + 1) * sizeof(bvh::MBVHNode),
                   m_MBVHTree->m_Tree.data());
    MBVHNodeBuffer->CopyToDevice();

    triangleBuffer = new Buffer(
        static_cast<unsigned int>(m_ObjectList->GetTriangles().size()) *
            sizeof(GpuTriangle),
...
void GpuTracer::SetupMaterials()
{
    materialBuffer = new Buffer(
        static_cast<unsigned int>(
            material::MaterialManager::GetInstance()->GetMaterials().size()) *
            sizeof(material::Material),
        (void *)material::MaterialManager::GetInstance()
            ->GetMaterials()
            .data());
    materialBuffer->CopyToDevice();

    microfacetBuffer = new Buffer(
        static_cast<unsigned int>(
            material::MaterialManager::GetInstance()->GetMicrofacets().size()) *
            sizeof(Microfacet),
        (void *)material::MaterialManager::GetInstance()
            ->GetMicrofacets()
            .data());
...
void GpuTracer::SetupNEEData()
{
    ...
    if (lightCount > 0)
    {
        lightIndices = new Buffer(lightCount * sizeof(unsigned int),
                                  &m_ObjectList->GetLightIndices()[0]);
        ...
        for (int i = 1; i < lightCount; i++)
        {
            lLotteryTickets[i] =
                lLotteryTickets[i - 1] +
                m_ObjectList->GetTriangle(i).m_Area / lightArea;
        }
        ...
        lightLotteryTickets =
            new Buffer(lightCount * sizeof(float), lLotteryTickets.data());
...
void GpuTracer::SetupTextures()
{
    ...
    if (!material::MaterialManager::GetInstance()->GetTextures().empty())
    {
        std::vector<TextureInfo> textureInfos{};
        unsigned int vec4Offset = 0;
        for (auto &tex :
             material::MaterialManager::GetInstance()->GetTextures())
        {
            const auto texSize = tex->GetWidth() * tex->GetHeight();
            textureInfos.emplace_back(tex->GetWidth(), tex->GetHeight(),
                                      vec4Offset);
            vec4Offset += texSize;
        }
        ...
        for (unsigned int i = 0; i < textureInfos.size(); i++)
        {
            ...
            auto tex =
                material::MaterialManager::GetInstance()->GetTextures()[i];
            memcpy(&textureColors[offset], tex->GetTextureBuffer(),
        ...
        textureInfoBuffer =
            new Buffer(static_cast<unsigned int>(textureInfos.size()) *
                           sizeof(TextureInfo),
                       textureInfos.data());
...
void GpuTracer::SetupSkybox(Surface *skyBox)
{
    ...
    if (m_HasSkybox)
    {
        ...
        skyDome = new Buffer(skyBox->GetWidth() * skyBox->GetHeight() *
                                 sizeof(glm::vec4),
                             skyBox->GetTextureBuffer());
