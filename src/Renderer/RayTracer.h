...
class RayTracer : public Renderer
{
  public:
    ...
    RayTracer(WorldScene *Scene, glm::vec3 backgroundColor,
              glm::vec3 ambientColor, uint maxRecursionDepth, Camera *camera,
              int width, int height);
    glm::vec3 Trace(Ray &r, uint &depth, float refractionIndex,
                    RandomGenerator &rng);
    glm::vec3 Shade(const Ray &r, uint &depth, float refractionIndex,
                    RandomGenerator &rng);
    glm::vec3 GetDiffuseSpecularColor(const Ray &r,
                                      const material::Material &mat,
                                      const glm::vec3 &hitPoint,
                                      RandomGenerator &rng) const;
    glm::vec3 GetreflectionRefractionColor(const Ray &r, uint &depth,
