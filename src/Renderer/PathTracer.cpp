      ...
      m_Camera(camera), m_SkyboxEnabled(skyBox != nullptr)
{
    ...
    m_Samples = 0;
    m_Materials = material::MaterialManager::GetInstance();
}
...
glm::vec3 PathTracer::SampleNEE(Ray &r, RandomGenerator &rng) const
{
    ...
    for (uint depth = 0; depth < LOOP_DEPTH; depth++)
    {
        ...
        const vec3 BRDF = albedoColor * INVPI;
...
glm::vec3 PathTracer::SampleIS(Ray &r, RandomGenerator &rng) const
{
    ...
    for (uint depth = 0; depth < LOOP_DEPTH; depth++)
    {
        ...
        const float PDF = dot(normal, r.direction) * INVPI;
        const glm::vec3 BRDF = albedoColor * INVPI;
        throughput *= BRDF * dot(normal, r.direction) / PDF;
...
glm::vec3 PathTracer::SampleNEE_IS(Ray &r, RandomGenerator &rng) const
{
    ...
    for (uint depth = 0; depth < LOOP_DEPTH; depth++)
    {
        ...
        r = r.CosineWeightedDiffuseReflection(p, normal, rng);
        const float PDF = dot(normal, r.direction) / PI;
        const glm::vec3 BRDF = albedoColor * INVPI;
...
void PathTracer::Resize(Texture *newOutput)
{
                                                ...
                                                RandomGenerator &rng) const
{
    ...
    for (uint depth = 0; depth < LOOP_DEPTH; depth++)
    {
        ...
        wiLocal = worldToLocalMicro(normal, r.direction, u, v, w);
        wmLocal = mf.sampleWm(rng);
        wm = localToWorldMicro(wmLocal, u, v, w);
