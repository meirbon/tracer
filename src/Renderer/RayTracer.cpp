...
RayTracer::RayTracer(WorldScene *Scene, glm::vec3 backGroundColor,
                     glm::vec3 ambientColor, uint maxRecursionDepth,
    ...
    for (int tile_y = 0; tile_y < vTiles; tile_y++)
    {
        for (int tile_x = 0; tile_x < hTiles; tile_x++)
        {
            ...
            tResults->at(tile_y * hTiles + tile_x) =
                tPool->push([tile_x, tile_y, this, output,
                             rngPointer](int threadId) -> void {
                    for (int y = 0; y < TILE_HEIGHT; y++)
                    {
                        ...
                        for (int x = 0; x < TILE_WIDTH; x++)
                        {
                            ...
                            {
                                const float factor =
                                    1.0f / float(m_Samples + 1);
                                m_Pixels[pidx] =
                                    m_Pixels[pidx] * float(m_Samples) * factor +
                                    color * factor;
                            }
                            else
                            {
...
glm::vec3 RayTracer::Trace(Ray &r, uint &depth, float refractionIndex,
                           RandomGenerator &rng)
{
    m_Scene->TraceRay(r);
...
glm::vec3 RayTracer::Shade(const Ray &r, uint &depth, float refractionIndex,
                           RandomGenerator &rng)
{
    ...
    const auto mat = material::MaterialManager::GetInstance()->GetMaterial(
        r.obj->materialIdx);
    ...
    const glm::vec3 diffuseColor =
        mat.IsDiffuse()
            ? GetDiffuseSpecularColor(r, mat, p, rng) * mat.GetDiffuse()
            : glm::vec3(0.0f);
    const glm::vec3 reflectionRefractionColor =
        mat.IsReflective() ? GetreflectionRefractionColor(
                                 r, depth, refractionIndex, mat, p, rng) *
                                 mat.GetSpecular()
                           : glm::vec3(0.0f);
...
glm::vec3 RayTracer::GetDiffuseSpecularColor(const Ray &r,
                                             ...
                                             RandomGenerator &rng) const
{
    ...
    const SceneObject *lights[3] = {m_Scene->GetLights().at(l1),
                                    m_Scene->GetLights().at(l2),
    ...
    for (auto *light : lights)
    {
        ...
        {
            glm::vec3 lNormal;
            const glm::vec3 pointOnLight =
                light->GetRandomPointOnSurface(towardsLightNorm, lNormal, rng);
            ...
            if (LNdotL > 0.0f)
            {
                ...
                const float SolidAngle =
                    LNdotL * (light->m_Area / towardsLightDot);

                ret = mat.GetAlbedoColor(r.obj, p) * lightMat.GetEmission() *
                      SolidAngle * NdotL * float(m_LightCount);
            }
        }
    }
...
glm::vec3 RayTracer::GetreflectionRefractionColor(const Ray &r, uint &depth,
                                                  ...
                                                  RandomGenerator &rng)
{
    ...
    Ray reflectionRay = r.Reflect(hitPoint, flipNormal ? -normal : normal);
    const glm::vec3 reflectiveColor =
        Trace(reflectionRay, depth, refractionIndex, rng);
    ...
    if (mat.IsTransparent())
    {
        ...
        if (k >= 0.f)
        {
            ...
            FractionReflection =
                R0 + (1.f - R0) * (minCosTheta * minCosTheta * minCosTheta *
    ...
    return reflectiveColor * FractionReflection +
           refractionCol * FractionTransmission;
}

void RayTracer::Resize(Texture *newOutput) {}
