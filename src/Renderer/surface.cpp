...
Surface::Surface(int width, int height)
    : m_Width(width), m_Height(height), m_Pitch(width)
{
...
void Surface::LoadImage(const char *file)
{
    ...
    for (auto y = 0; y < m_Height; y++)
    {
        for (auto x = 0; x < m_Width; x++)
        {
            ...
            m_Buffer[x + y * m_Width] =
                (red << 0) | (green << 8) | (blue << 16);
            m_TexBuffer[x + y * m_Width] =
                glm::vec4(float(red) / 255.0f, float(green) / 255.0f,
                          float(blue) / 255.0f, 1.0f);
        }
    }
...
void Surface::Resize(Surface *original)
{
    ...
    for (v = 0; v < m_Height; v++)
    {
        for (u = 0; u < m_Width; u++)
        {
            ...
            Pixel p1 = *s, p2 = *(s + x2), p3 = *(s + owidth * y2),
                  p4 = *(s + owidth * y2 + x2);
            unsigned int r = (((p1 & REDMASK) * w1 + (p2 & REDMASK) * w2 +
                               (p3 & REDMASK) * w3 + (p4 & REDMASK) * w4) >>
                              8) &
                             REDMASK;
            unsigned int g = (((p1 & GREENMASK) * w1 + (p2 & GREENMASK) * w2 +
                               (p3 & GREENMASK) * w3 + (p4 & GREENMASK) * w4) >>
                              8) &
                             GREENMASK;
            unsigned int b = (((p1 & BLUEMASK) * w1 + (p2 & BLUEMASK) * w2 +
                               (p3 & BLUEMASK) * w3 + (p4 & BLUEMASK) * w4) >>
                              8) &
                             BLUEMASK;
...
{
    const int yValue =
        (int)glm::max(0, (int)(glm::min(texCoords.y, 1.f) * m_Height) - 1);
    const int xValue =
        (int)glm::max(0, (int)(glm::min(texCoords.x, 1.f) * m_Width) - 1);
...
void Surface::Line(float x1, float y1, float x2, float y2, Pixel c)
{
    ...
    const float xmin = 0, ymin = 0, xmax = (float)m_Width - 1,
                ymax = (float)m_Height - 1;
...
void Surface::ScaleColor(unsigned int scale)
{
    ...
    for (int i = 0; i < s; i++)
    {
        Pixel c = m_Buffer[i];
        unsigned int rb =
            (((c & (REDMASK | BLUEMASK)) * scale) >> 5) & (REDMASK | BLUEMASK);
...
Sprite::Sprite(Surface *output, unsigned int numFrames)
    : m_Width(output->GetWidth() / numFrames), m_Height(output->GetHeight()),
      m_Pitch(output->GetWidth()), m_NumFrames(numFrames), m_CurrentFrame(0),
      m_Flags(0), m_Start(new unsigned int *[numFrames]), m_Surface(output)
{
    InitializeStartData();
}
...
void Sprite::DrawScaled(int X, int Y, int width, int height, Surface *output)
{
    ...
    for (int x = 0; x < width; x++)
        for (int y = 0; y < height; y++)
        {
            ...
            if (color & 0xffffff)
                output->GetBuffer()[X + x + ((Y + y) * output->GetPitch())] =
                    color;
        }
}
...
void SurfaceFont::Print(Surface *output, const char *text, int X, int Y,
                        bool clip)
{
