...
void Micromaterials(TriangleList *objectList)
{
	...
	const auto lightMat = Material::light(vec3(5.0f));
	...
	const auto sphereIdx = mManager->AddMaterial(sphereMat);
	const auto yellowIdx = mManager->AddMaterial(yellowMat);
	const auto redIdx = mManager->AddMaterial(redMat);
	const auto lightIdx = mManager->AddMaterial(lightMat);
	...
	objectList->loadModel(
		...
		glm::rotate(glm::translate(glm::mat4(1.0f), vec3(0, 0, -10)), glm::radians(-90.f), vec3(1, 0, 0)), dragIdx,
		true);
}
